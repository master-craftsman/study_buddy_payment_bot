...
`Ученик`, `Ученик из абонемента` и `Дата урока` в `Участиях` подтягиваются автоматически. После урока администратор меняет только `Статус участия` на `Был`, `Сгорело`, `Отмена без списания` или `Перенос`. Ручно поле `Ученик` заполнять не нужно; оно используется для группировки представления `По ученикам`.

В `Занятиях` представление `По абонементам` группирует расписание по `Абонементы занятия` и сортирует занятия по дате. Это поле является источником состава занятия: если убрать из него абонемент, скрипт не добавит его обратно.
